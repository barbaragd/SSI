...
void vernam::set_key (string new_key){ // pasar el string de la llave a un vector de bitset
    bitset<8> k;
    istringstream key (new_key);
    for (int i=0; i< new_key.size() / 8; i++){
        key >> k;
        key_.push_back(k);
    }
}

void vernam::set_random_key (int l){ // establecer una llave aleatoria y mostrarla
    srand (time(NULL));
    int clave;
    for(int i=0; i<l / 8; i++){
        clave = rand() % 63;
...
string vernam::cifrado (string msj_original){
    ...
    // Suma XOR
    for(int i=0; i<msj_bin_.size(); i++){
        result_sum[i] ^=key_[i];
        if(result_sum[i].to_ulong() > 126){ 
            int m = result_sum[i].to_ulong() % 126;
            bitset<8> aux(m);            
            result_sum[i] = aux;
        }
        if(result_sum[i].to_ulong() < 33){ 
            unsigned m = result_sum[i].to_ulong() + 33;
            bitset<8> aux(m);
            result_sum[i] = aux;
        }
    }
...
string vernam::descifrado (string msj_cifrado){
    ...
    // Suma XOR
    for(int i=0; i<msj_cifrado.size(); i++){
        result_sum[i] ^=key_[i];
        if(result_sum[i].to_ulong() > 126){ 
            int m = result_sum[i].to_ulong() % 126;
            bitset<8> aux(m);            
...
string vernam::get_msj_cifrado_bin(){ // Mensaje cifrado en binario
    string msj;
    for(int i=0; i<msj_cifrado_bin_.size(); i++){
...
string vernam::get_msj_descifrado_bin(){ // Mensaje descifrado en binario
    string msj;
    for(int i=0; i<msj_descifrado_bin_.size(); i++){
